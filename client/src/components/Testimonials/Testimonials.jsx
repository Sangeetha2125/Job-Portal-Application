...
function Testimonials() {
  ...
  return (
    <div className='flex flex-col gap-4 md:px-[1rem] px-[1rem]'>
      ...
      <div className='relative w-full flex justify-center items-center'>
        <div className='absolute left-[-30px] top-1/2 transform -translate-y-1/2'>
          <button 
            onClick={handlePrev} 
            className='bg-transparent text-darkblue p-0 rounded-full z-10 transition duration-300'
          >
          ...
          <div className='flex transition-transform duration-500 ease-in-out gap-3'>
            {getVisibleTestimonials().map((testimonial, index) => (
              <div key={index} className='flex-shrink-0 w-full md:w-1/3 p-2'>
                <div className='w-full p-4 pb-6 bg-white rounded-md shadow-md border border-darkblue'>
                  <div className='flex items-center gap-4 p-2'>
                    <img src={testimonial.image} alt={testimonial.name} className='w-16 h-16 rounded-full' />
                    <div>
                      <p className='text-lg font-semibold text-darkblue'>{testimonial.name}</p>
