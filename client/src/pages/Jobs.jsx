...
export const Jobs = () => {
  ...
  const companyData = ["Google", "Apple", "Paypal", "Samsung", "Amazon", "Oracle"];
  ...
  for (let i = startButton; i <= endButton; i++) {
    pageButtons.push(
      <button
        ...
        className={`mx-1 px-3 py-1 border border-gray-700 rounded ${currentPage === i ? 'bg-blue-800 text-white' : 'bg-white text-gray-800 hover:bg-indigo-100'}`}
      >
        {i}
      </button>
    );
  ...
  return (
    <>
      <MetaData title="Jobs" />
      <div className='bg-gray-50 min-h-screen pt-14 sm:px-20 px-3 text-gray-800'>

        {loading ? <Loader /> : <>
          <div className='flex-col flex justify-center items-center w-full '>
            <div className='text-center pt-8 sm:text-3xl text-2xl font-medium my-4'>
              <p>Find your dream job now</p>
            </div>
            <div className='py-3 mb-4 pt-4 w-full flex justify-center items-center'>

              <div className='flex justify-center w-full items-center '>
                <div className='bg-white flex sm:w-2/5 w-4/5 rounded-md'>
                  <div className='flex justify-center items-center pl-2 text-gray-800'>
                    <FiSearch size={19} />
                  </div>
                  <input
                    value={search}
                    ...
                    type="text"
                    className='outline-none px-2 pl-3 sm:h-10 w-full h-8 py-1 text-sm text-gray-800'
                  />
                  <div className='text-gray-800 items-center flex justify-center px-2'>
                    <RxCross2 onClick={() => setSearch("")} size={19} className={`cursor-pointer ${search.length !== 0 ? "flex" : "hidden"}`} />
                  </div>
                  <button onClick={searchHandler} className='bg-blue-800 text-white sm:text-sm text-xs px-4 sm:h-10 h-8 py-1 rounded-md'>
                    Search
                  </button>
                </div>
              </div>
              ...
              {!isMobile && <div className='filter1 flex flex-col'>
                <div className='flex justify-start flex-col filter'>
                  ...
                  <ul className='flex pt-3 flex-col gap-3'>
                    {data.map((e, i) => (
                      <li key={i} onClick={() => setCategory(e)} className={`hover:text-blue-600 cursor-pointer ${category === e ? "text-blue-600" : ""}`}>
                        {e}
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className='flex flex-col gap-4 w-2/3 pt-5'>
                    <button onClick={() => leftFilter(jobs)} className='bg-blue-800 text-white px-1 py-1 text-xs rounded-md'>
                      Apply Filter
                    </button>
                    <button onClick={removeLeftFilter} className='bg-blue-800 text-white px-1 py-1 text-xs rounded-md'>
                      Remove Filter
                    </button>
                  </div>
                </div>
              ...
              <div className='sm:w-2/4 pb-20 pt-2'>
                ...
                <div className="flex justify-center mt-6">
                  <button
                    ...
                    className="bg-blue-800 text-white font-bold py-2 px-4 mr-2 rounded-md"
                  >
                    Previous
                  </button>
                  {pageButtons}
                  <button
                    ...
                    className="bg-blue-800 text-white font-bold py-2 px-4 ml-2 rounded-md"
                  >
                    Next
                  </button>
                </div>
              </div>
              <div className='filter2 flex flex-col'>
                <div className='flex justify-start flex-col filter'>
                  <p className='text-xl underline underline-offset-4'>Companies</p>
                  <ul className='flex pt-3 flex-col gap-3'>
                    {companyData.map((e, i) => (
                      <li key={i} onClick={() => setCompany(e)} className={`hover:text-blue-600 cursor-pointer ${company === e ? "text-blue-600" : ""}`}>
                        {e}
                      </li>
                    ))}
                  </ul>
                  <div className='flex flex-col gap-4 pt-5'>
                    <button onClick={() => rightFilter(jobs)} className='bg-blue-800 text-white px-1 py-1 text-xs rounded-md'>
                      Apply Filter
                    </button>
                    <button onClick={removeRightFilter} className='bg-blue-800 text-white px-1 py-1 text-xs rounded-md'>
                      Remove Filter
                    </button>
                  </div>
                </div>
